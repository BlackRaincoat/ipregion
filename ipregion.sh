...
SCRIPT_URL="https://github.com/BlackRaincoat/ipregion"
...
run_service_group() {
  ...
  for service_name in "${services_array[@]}"; do
    ...
    if [[ -n "$SERVICES_FILTER" ]]; then
      local IFS_BAK="$IFS"
      IFS=',' read -ra _filter_arr <<<"$SERVICES_FILTER"
      IFS="$IFS_BAK"
      local _matched=false
      for _s in "${_filter_arr[@]}"; do
        [[ "${_s^^}" == "$service_name" ]] && _matched=true && break
      done
      if [[ "$_matched" == false ]]; then
        log "$LOG_INFO" "Skipping service (not in filter): $service_name"
        continue
      fi
    fi
...
print_header() {
  ...
  printf "%s\n%s\n\n" "$(color URL "Forked by BlackRaincoat")" "$(color URL "$SCRIPT_URL")"
...
main() {
  ...
  if [[ -n "$SERVICES_FILTER" && "$GROUPS_TO_SHOW" == "all" ]]; then
